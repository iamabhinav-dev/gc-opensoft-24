...
const HLSPlayer = ({src,name}) => {
    ...
    return (
        <div className={`max-h-screen w-full object-contain relative`}>
            <div className={`absolute text-white z-[990] top-0 w-full h-[30%] flex justify-center items-center text-4xl font-semibold bg-[skyblue] fade-overlay-down pointer-events-none duration-200 ${!mouseMove?"translate-y-[-100%]":""}`}>
            </div>
            ...
            <video ref={videoRef} className={`max-h-screen object-cover w-full`} id="player" controls></video>
        </div>
    );
};
