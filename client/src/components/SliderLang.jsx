...
function SliderLang({ lang }) {
  ...
  const fetchData = async () => {
    try {
      const result = await axios.post(`/lang`,
        JSON.stringify({
          lang: lang,
          lim: 20
        }),
  ...
  return (
    <div className="flex flex-col justify-center mt-11 px-6">
      <div className="flex items-center gap-7 text-2xl mb-6">
        {sub && <span className="font-bold text-[#1A98FF]">Prime</span>}
        <span className="font-bold">Movies in {lang}</span>
        <button onClick={() => {
          lang = lang.toLowerCase();
          navigate(`/lang/${lang}`, { state: { lang: lang } });
        }}>See More</button>
      </div>
