...
function FullSearch({ query }) {
    ...
    const fetchData = async () => {
        try {
            const result = await axios.post(`/search/advanced`,
                JSON.stringify({
                    query: query,
                }),
                {
    ...
    return (
        <div className="flex flex-col justify-center mt-7 mb-10 mx-10 pt-[80px]">
            <div className="flex items-center gap-7 text-4xl mb-8">
                ...
                <span className="font-bold">Advanced Search for <span className="text-sky-400 italic">{query}</span></span>
            </div>
            <div className="flex justify-start pl-[30px] items-center gap-8 relative flex-wrap">
                {
